import { View, Dimensions, SafeAreaView } from "react-native";
import React, { useRef, useState } from "react";
import { Button, Stack, XStack, Text, YStack } from "tamagui";
import { Image } from "expo-image";
import Animated, {
  useAnimatedStyle,
  useSharedValue,
  withSpring,
} from "react-native-reanimated";
import ExitButton from "../../components/ExitButton";
import * as Burnt from "burnt";
import { buttonBlue } from "../../utils/colors";
const { width, height } = Dimensions.get("screen");

const AnimatedImage = Animated.createAnimatedComponent(Image);

const TicTacToeGame = () => {
  const [button, setButton] = useState(1);
  // 1 - X
  // 2 - O
  const [xox, setXox] = useState([
    [0, 0, 0],
    [0, 0, 0],
    [0, 0, 0],
  ]);
  const [gameover, setGameover] = useState(false);
  const swichPos = useSharedValue(-50);
  const imageAni = useSharedValue(0.7);
  const rotatePos = useSharedValue("180");

  const _handlePress = ({ rowIndex, colIndex }) => {
    swichPos.value = swichPos.value == 50 ? -50 : 50;

    rotatePos.value = rotatePos.value == "0" ? "180" : "0";
    let newXox = [...xox];
    newXox[rowIndex][colIndex] = button;
    setXox(newXox);

    if (
      (newXox[rowIndex][0] == newXox[rowIndex][1] &&
        newXox[rowIndex][1] == newXox[rowIndex][2]) ||
      (newXox[0][colIndex] == newXox[1][colIndex] &&
        newXox[1][colIndex] == newXox[2][colIndex]) ||
      (((newXox[0][0] == newXox[1][1] && newXox[1][1] == newXox[2][2]) ||
        (newXox[0][2] == newXox[1][1] && newXox[1][1] == newXox[2][0])) &&
        newXox[1][1] != 0)
    ) {
      setGameover(true);
      imageAni.value = 1;
      Burnt.toast({
        duration: 2,
        haptic: "warning",
        preset: "done",
        title: button == 1 ? "X kazandı." : "O kazandı.",
      });
    } else {
      setButton(button == 1 ? 2 : 1);
    }
  };
  const animatedStyle = useAnimatedStyle(() => {
    "worklet";
    return {
      marginLeft: withSpring(swichPos.value),
      transform: [{ rotateY: withSpring(`${rotatePos.value}deg`) }],
    };
  });
  const imageAniStyle = useAnimatedStyle(() => {
    "worklet";
    return {
      transform: [{ scale: withSpring(imageAni.value) }],
    };
  });

  return (
    <>
      <ExitButton />
      <SafeAreaView
        style={{
          flex: 1,
          alignItems: "center",
          justifyContent: "center",
          backgroundColor: "white",
        }}
      >
        <Stack ai="center">
          <YStack zIndex={1}>
            {xox.map((item, index) => {
              const idx = index;
              return (
                <XStack zIndex={1} key={idx}>
                  {item.map((item, index) => {
                    return (
                      <Button
                        m={2}
                        alignItems="center"
                        jc="center"
                        p={0}
                        key={index}
                        bg={"white"}
                        height={Math.floor(height / 8.1)}
                        width={Math.floor(width / 3.0)}
                        onPress={() =>
                          _handlePress({ rowIndex: idx, colIndex: index })
                        }
                        disabled={item != 0 || gameover}
                      >
                        <Text fos={40} fow={"700"}>
                          {item == 1 ? "X" : item == 2 ? "O" : null}
                        </Text>
                      </Button>
                    );
                  })}
                </XStack>
              );
            })}
          </YStack>
          <YStack zIndex={0} pos="absolute" alignItems="center" jc="center">
            <XStack>
              <Stack
                height={Math.floor(height) / 8}
                width={Math.floor(width) / 3}
              />
              <Stack
                bg={buttonBlue}
                height={Math.floor(height) / 8}
                width={4}
              />
              <Stack
                height={Math.floor(height) / 8}
                width={Math.floor(width) / 3}
              />
              <Stack
                bg={buttonBlue}
                height={Math.floor(height) / 8}
                width={4}
              />
              <Stack
                height={Math.floor(height) / 8}
                width={Math.floor(width) / 3}
              />
            </XStack>
            <Stack bg={buttonBlue} height={4} width={Math.floor(width) - 30} />
            <XStack>
              <Stack
                height={Math.floor(height) / 8}
                width={Math.floor(width) / 3}
              />
              <Stack
                bg={buttonBlue}
                height={Math.floor(height) / 8}
                width={4}
              />
              <Stack
                height={Math.floor(height) / 8}
                width={Math.floor(width) / 3}
              />
              <Stack
                bg={buttonBlue}
                height={Math.floor(height) / 8}
                width={4}
              />
              <Stack
                height={Math.floor(height) / 8}
                width={Math.floor(width) / 3}
              />
            </XStack>
            <Stack bg={buttonBlue} height={4} width={Math.floor(width) - 30} />
            <XStack>
              <Stack
                height={Math.floor(height) / 8}
                width={Math.floor(width) / 3}
              />
              <Stack
                bg={buttonBlue}
                height={Math.floor(height) / 8}
                width={4}
              />
              <Stack
                height={Math.floor(height) / 8}
                width={Math.floor(width) / 3}
              />
              <Stack
                bg={buttonBlue}
                height={Math.floor(height) / 8}
                width={4}
              />
              <Stack
                height={Math.floor(height) / 8}
                width={Math.floor(width) / 3}
              />
            </XStack>
          </YStack>
        </Stack>
        <XStack mt={40} ai="center">
          <Image
            style={{
              width: 40,
              height: 40,
            }}
            contentFit="cover"
            source={require("../../assets/x.png")}
          />
          <XStack
            ai={"center"}
            jc="center"
            mx={10}
            width={120}
            height={60}
            br={30}
            bg={"white"}
            shadowColor={"#000"}
            shadowOffset={{
              width: 0,
              height: 3,
            }}
            shadowOpacity={0.29}
            shadowRadius={4.65}
            elevationAndroid={7}
          >
            <Animated.View
              style={[
                animatedStyle,
                {
                  alignItems: "center",
                  justifyContent: "center",
                  width: 48,
                  height: 48,
                  borderRadius: 24,
                  backgroundColor: "white",
                  shadowColor: "#000",
                  shadowOffset: {
                    width: 0,
                    height: 3,
                  },
                  shadowOpacity: 0.29,
                  shadowRadius: 4.65,

                  elevation: 7,
                },
              ]}
            >
              <Image
                style={{
                  width: 35,
                  height: 35,
                }}
                contentFit="cover"
                source={require("../../assets/arrow.png")}
              />
            </Animated.View>
          </XStack>
          <AnimatedImage
            style={[
              {
                width: 40,
                height: 40,
              },
            ]}
            contentFit={"cover"}
            source={require("../../assets/o.png")}
          />
        </XStack>
        <Button
          width={100}
          height={100}
          br={50}
          fd="column"
          ai="center"
          jc="center"
          p={5}
          bg={"white"}
          mt={20}
          shadowColor={"#000"}
          shadowOffset={{
            width: 0,
            height: 2,
          }}
          shadowOpacity={0.23}
          shadowRadius={2.63}
          elevationAndroid={4}
        >
          <AnimatedImage
            source={require("../../assets/refresh.png")}
            contentFit={"fill"}
            style={[
              imageAniStyle,
              {
                width: 50,
                height: 50,
              },
            ]}
          />
          <Text color={buttonBlue}>Tekrar</Text>
        </Button>
      </SafeAreaView>
    </>
  );
};

export default TicTacToeGame;
